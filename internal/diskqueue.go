...
func New(name string, dataPath string, maxBytesPerFile int64,
	...
	syncEvery int64, syncTimeout time.Duration) Queue {
	d := diskQueue{
		name:              name,
		dataPath:          dataPath,
	...
	if err != nil && !os.IsNotExist(err) {
		DefaultLogger.Errorf("DISKQUEUE(%s) failed to retrieveMetaData - %s", d.name, err)
	}
...
func (d *diskQueue) exit(deleted bool) error {
	...
	if deleted {
		DefaultLogger.Infof("DISKQUEUE(%s): deleting", d.name)
	} else {
		DefaultLogger.Infof("DISKQUEUE(%s): closing", d.name)
	}
...
func (d *diskQueue) Empty() error {
	...
	DefaultLogger.Infof("DISKQUEUE(%s): emptying", d.name)
...
func (d *diskQueue) deleteAllFiles() error {
	...
	if innerErr != nil && !os.IsNotExist(innerErr) {
		DefaultLogger.Errorf("DISKQUEUE(%s) failed to remove metadata file - %s", d.name, innerErr)
		return innerErr
	}
...
func (d *diskQueue) skipToNextRWFile() error {
	...
	for i := d.readFileNum; i <= d.writeFileNum; i++ {
		...
		if innerErr != nil && !os.IsNotExist(innerErr) {
			DefaultLogger.Errorf("DISKQUEUE(%s) failed to remove data file - %s", d.name, innerErr)
			err = innerErr
		}
	}
...
func (d *diskQueue) readOne() ([]byte, error) {
	...
	if d.readFile == nil {
		...
		DefaultLogger.Infof("DISKQUEUE(%s): readOne() opened %s", d.name, curFileName)
...
func (d *diskQueue) writeOne(data []byte) error {
	...
	if d.writePos > 0 && d.writePos+totalBytes > d.maxBytesPerFile {
		...
		err = d.sync()
		if err != nil {
			DefaultLogger.Errorf("DISKQUEUE(%s) failed to sync - %s", d.name, err)
		}
	...
	if d.writeFile == nil {
		...
		DefaultLogger.Infof("DISKQUEUE(%s): writeOne() opened %s", d.name, curFileName)
...
func (d *diskQueue) moveForward() {
	...
	if oldReadFileNum != d.nextReadFileNum {
		...
		err := os.Remove(fn)
		if err != nil {
			DefaultLogger.Errorf("DISKQUEUE(%s) failed to Remove(%s) - %s", d.name, fn, err)
		}
	}
	...
	for {
		...
		if (d.readFileNum < d.writeFileNum) || (d.readPos < d.writePos) {
			if d.nextReadPos == d.readPos {
				...
				if err != nil {
					DefaultLogger.Errorf("DISKQUEUE(%s) reading at %d of %s - %s",
...
exit:
	DefaultLogger.Infof("DISKQUEUE(%s): closing ... ioLoop", d.name)
	syncTicker.Stop()
	d.exitSyncChan <- 1
}
